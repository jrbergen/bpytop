...
from src.config.config_cls import Config
from src.static.directories import Paths
from src.logger import ErrorLogger
from src.utils import get_system_string, TimeIt
from static.appearance import BANNER_SRC

__VERSION__: str = "1.0.64"

SELF_START = time()
SYSTEM: str = get_system_string()

IMPORTS = ['fcntl', 'termios', 'tty', 'pwd', 'psutil']
errors = []
...
# ? Argument parser ------------------------------------------------------------------------------->
cli_args = CliArgs()
ARG_BOXES: str = cli_args.ARG_BOXES
LOW_COLOR: bool = cli_args.LOW_COLOR
...
try:
    CONFIG: Config = Config(Paths.CONFIG_FILE)
    if DEBUG:
        errlog.setLevel(logging.DEBUG)
    else:
        errlog.setLevel(getattr(logging, CONFIG.log_level))
        DEBUG = CONFIG.log_level == "DEBUG"
    errlog.info(f'New instance of bpytop version {__VERSION__} started with pid {os.getpid()}')
    ...
    def empty_bg(cls) -> str:
        return (f'{Term.clear}' +
                (f'{Banner.draw(Term.height // 2 - 10, center=True)}'
                 ...
                 f'{Mv.r(25)}{Fx.i}Version: {__VERSION__}{Fx.ui}' if Term.height > 22 else "") +
    ...
    def _draw_fg(cls):
        ...
        if cls.resized or cls.redraw:
            ...
            if w - len(net.nic[:10]) - 20 > 6:
                if not "a" in Key.mouse: Key.mouse["a"] = [[x + w - 20 - len(net.nic[:10]) + i, y - 1] for i in
                                                           range(4)]
                out_misc += (
            ...
            if w - len(net.nic[:10]) - 20 > 13:
                if not "y" in Key.mouse: Key.mouse["y"] = [[x + w - 26 - len(net.nic[:10]) + i, y - 1] for i in
                                                           range(4)]
                out_misc += (
        ...
        while not cls.close:
            ...
            if cls.resized:
                banner = (
                    ...
                    f'{Mv.r(30)}{Fx.i}Version: {__VERSION__}{Fx.ui}{Fx.ub}{Term.bg}{Term.fg}')
                if UpdateChecker.version != __VERSION__:
...
class UpdateChecker:
    ...
    def _checker(cls):
        try:
            ...
            if cls.version != __VERSION__ and which("notify-send"):
                try:
...
class Init:
    ...
    def start(cls):
        ...
        Draw.buffer("banner", (
            ...
            f'{Fx.b}{Fx.i}Version: {__VERSION__}{Fx.ui}{Fx.ub}{Term.bg}{Term.fg}{Color.fg("#50")}'), z=2)
        for _i in range(7):
    ...
    def success(cls):
        ...
        cls.draw_bg(5)
        Draw.buffer(cls.STR_INIT, f'{Mv.restore}{Symbol.ok}\n{Mv.r(Term.width // 2 - 22)}{Mv.save}')

    @staticmethod
    def fail(err):
        if CONFIG.show_init:
            Draw.buffer(cls.STR_INIT, f'{Mv.restore}{Symbol.fail}')
            sleep(2)
...
def floating_humanizer(value: Union[float, int], bit: bool = False,
                       per_second: bool = False, start: int = 0, short: bool = False) -> str:
...
def process_keys():
    ...
    MOUSE_OUTSIDE_PROCBOX = \
        mouse_pos[0] >= ProcBox.x and ProcBox.current_y + 1 <= mouse_pos[1] < ProcBox.current_y + ProcBox.current_h - 1
...
def main():
    ...
    # ? Load theme
    if CONFIG.show_init:
        Draw.buffer(Init.STR_INIT, f'{Mv.restore}{Fx.trans("Loading theme and creating colors... ")}{Mv.save}')
    try:
    ...
    # ? Setup boxes
    if CONFIG.show_init:
        Draw.buffer(Init.STR_INIT, f'{Mv.restore}{Fx.trans("Doing some maths and drawing... ")}{Mv.save}')
    try:
    ...
    if CONFIG.show_init:
        Draw.buffer(Init.STR_INIT, f'{Mv.restore}{Fx.trans("Setting up signal handlers... ")}{Mv.save}')
    try:
    ...
    if CONFIG.show_init:
        Draw.buffer(Init.STR_INIT, f'{Mv.restore}{Fx.trans("Starting input reader thread... ")}{Mv.save}')
    try:
    ...
    if CONFIG.show_init:
        Draw.buffer(Init.STR_INIT, f'{Mv.restore}{Fx.trans("Starting data collection and drawer thread... ")}{Mv.save}')
    try:
        Collector.start()
    except Exception as e:
        Init.fail(e)
    ...
    if CONFIG.show_init:
        Draw.buffer(Init.STR_INIT, f'{Mv.restore}{Fx.trans("Collecting data and drawing... ")}{Mv.save}')
    try:
    ...
    # ? Draw to screen
    if CONFIG.show_init:
        Draw.buffer(Init.STR_INIT, f'{Mv.restore}{Fx.trans("Finishing up... ")}{Mv.save}')
    try:
